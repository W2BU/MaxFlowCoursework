...
class MyMap
{
    ...
    public:
        ...
        void remove(Key key);
        Value findByKey(Key key);
        Key findByValue(Value value);
        bool has(Value value);
    ...
    private:
        ...
        TreeNode<Key, Value>* valueFindHelper(TreeNode<Key, Value>* node, const Value& value);
...
//  exception
template <typename Key, typename Value>
Value MyMap<Key, Value>::findByKey(Key key)
{
...
//  Finds the node with the given value and returns its key otherwise throws
//  exception
template <typename Key, typename Value>
Key MyMap<Key, Value>::findByValue(Value value)
{
    TreeNode<Key, Value>* current = valueFindHelper(this->root, value);
    if (current == nil) {
        throw std::invalid_argument("Key not found");
    } else {
        return current->key;
...
//  Checks if the node with the given value is in the tree
template <typename Key, typename Value>
bool MyMap<Key, Value>::has(Value value)
{
    TreeNode<Key, Value>* current = valueFindHelper(this->root, value);
    if (current == nil) {
        return false;
...
//  Traversal helper with time compexity of O(N)
template <typename Key, typename Value>
TreeNode<Key, Value>* MyMap<Key, Value>::valueFindHelper(TreeNode<Key, Value>* node, const Value& value)
{
    if (node != nil) {
        if (node->value == value) {
            return node;
        } else {
            TreeNode<Key, Value>* found = valueFindHelper(node->left, value);
            if (found == nil) {
               found = valueFindHelper(node->right, value);
            }
            return found;
        }
    } else {
...
void MyMap<Key, Value>::addToValue(Key key, int add)
{
    TreeNode<Key, Value>* current = this->root, *find = nil;
    while (current != nil) {
        if (current->value == key) find = current;
        if (current->key <= key) {
            current = current->right;
        } else {
            current = current->left;
